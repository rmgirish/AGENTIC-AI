...
class ConversationStore:
    ...
    def append_message(self, thread_id: str, role: str, text: str) -> int:
        """Insert a message with seq = highest seq + 1. Return the seq."""
        self.conn.execute(
            """INSERT INTO message (thread_id, seq, role, text)
               VALUES (?, COALESCE((SELECT MAX(seq)+1 FROM message WHERE thread_id=?), 1), ?, ?)""",
            (thread_id, thread_id, role, text)
        )
        seq = self.conn.execute("SELECT MAX(seq) FROM message WHERE thread_id=?", (thread_id,)).fetchone()[0]
        return seq

    def load_history(self, thread_id: str) -> list[dict]:
        """Return [{"seq","role","text"},...] in seq order."""
    ...
    def start_run(self, thread_id: str, model: str) -> str:
        """New run with uuid4 id and status 'running'. Return id."""
        run_id = str(uuid.uuid4())
        self.conn.execute(
            "INSERT INTO run (id, thread_id, model, status, started_at) VALUES (?, ?, ?, 'running', CURRENT_TIMESTAMP)",
            (run_id, thread_id, model)
        )
        return run_id

    def record_model_step(self, run_id: str, seq: int, tokens_in: int, tokens_out: int) -> int:
        """Insert a 'model' run_step and add tokens to run in one transaction."""
    ...
    def record_tool_call(self, run_id: str, seq: int, name: str, args: dict, result: dict,
        ...
        """Insert a 'tool' run_step and its tool_call in one transaction."""
        with self.transaction() as conn:
            cur = conn.execute(
    ...
    def finish_run(self, run_id: str, status: str, error_code: str | None = None) -> None:
        ...
        self.conn.execute(
            "UPDATE run SET status=?, error_code=?, finished_at=CURRENT_TIMESTAMP WHERE id=?",
    ...
    def page_messages(self, thread_id: str, after_seq: int = 0, limit: int = 20) -> tuple[list[dict], int | None]:
        """Return up to limit messages after after_seq, plus next cursor."""
        if limit < 1:
            raise ValueError("limit must be >=1")
        rows = self.conn.execute(
            """SELECT seq, role, text, created_at
               FROM message WHERE thread_id=? AND seq>? ORDER BY seq LIMIT ?""",
        ...
        if not msgs or len(msgs) < limit:
            ...
            next_after = msgs[-1]["seq"]
        return msgs, next_after
